...
fn lookup_project_file(dir: &Path) -> Result<Option<PathBuf>> {
    ...
    loop {
        ...
        match fs::metadata(&config_file) {
            ...
            // `rustfmt.toml`.
            Ok(md) if md.is_file() => return Ok(Some(config_file),
            ...
            Err(e) if e.kind() != ErrorKind::NotFound => return Err(From::from(e)),
        }
...
fn update_config(config: &mut Config, matches: &Matches) -> Result<()> {
    ...
    match matches.opt_str("write-mode").map(|wm| WriteMode::from_str(&wm)) {
        ...
        Some(Err(_)) => {
            Err(From::from(format!("Invalid write-mode: {}", write_mode.expect("cannot happen"))))
        }
    }
}
